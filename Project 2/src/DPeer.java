...
public class DPeer extends Peer{
	...
	public void run() {
			try{
				...
				System.out.println("new thread - downloading from " + ip + ":" + port);
		...
		while(true){
			switch(messageID){
				...
				case RUBTClientConstants.MESSAGE_TYPE_CHOKE :
					...
					this.isChoked = true;
					socket.setSoTimeout(120000);						//wait up to two minutes to get unchoked, if not sever connection
	 ...
	 * 
	 * @author Kevin Critelli
	 * @param x Message Object containing the message in bytes to send
	 * @throws Exception An Exception object is thrown when an error occurs
	 * */
