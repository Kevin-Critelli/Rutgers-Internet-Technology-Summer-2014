...
public final class Bencoder2 {
	 ...
	 *            the bencoded data to decode.
	 * @param offset
	 *            the offset into {@code bencoded_bytes} at which to start
	 *            decoding.
	 * @return a {@code Map}, {@code List}, {@code ByteBuffer}, or
	 *         {@code Integer}.
	 * @throws BencodingException
	 *             if the bencoded object in {@code bencoded_bytes} at offset
	 *             {@code offset} is incorrectly encoded.
	 */
	 ...
	 * @param offset
	 *            the position of the 'i' indicating the start of the bencoded
	 *            integer to be bdecoded.
	 * @return an <code>Object[]</code> containing an <code>Integer</code>
	 *         offset and the decoded <code>Integer</code>, in positions 0 and
	 *         1, respectively
	 * @throws BencodingException
	 *             if the bencoded integer in {@code bencoded_bytes} at offset
	 *             {@code offset} is incorrectly encoded.
	 */
	private static final Object[] decodeInteger(byte[] bencoded_bytes,
		...
		offset++;
		for (; bencoded_bytes[offset] != (byte) 'e'
				&& bencoded_bytes.length > (offset); offset++) {
			if ((bencoded_bytes[offset] < 48 || bencoded_bytes[offset] > 57)
					&& bencoded_bytes[offset] != 45)
				throw new BencodingException(
						"Expected an ASCII integer character, found "
	 ...
	 * @param offset
	 *            the offset into {@code bencoded_bytes} where the byte string
	 *            begins.
	 * @return an <code>Object[]</code> containing an <code>Integer</code>
	 *         offset and the decoded byte string (as a {@code ByteBuffer}), in
	...
	private static final Object[] decodeString(byte[] bencoded_bytes, int offset)
		...
		if (bencoded_bytes[offset] != ':') {
			throw new BencodingException(
					"Error: Invalid character at position " + offset
							+ ".\nExpecting ':' but found '"
							+ (char) bencoded_bytes[offset] + "'.");
		}
		offset++;
	 ...
	 * @return an <code>Object[]</code> containing an <code>Integer</code>
	 *         offset and the decoded list (as a {@code List}), in positions 0
	 *         and 1, respectively
	 * @throws BencodingException
	 *             if the bencoded object is incorrectly encoded.
	 */
	 ...
	 * @param offset
	 *            the offset into {@code bencoded_bytes} where the dictionary
	 *            begins.
	 * @return an <code>Object[]</code> containing an <code>Integer</code>
	 *         offset and the decoded dictionary (as a {@code Map}, in positions
	 *         0 and 1, respectively
	 * @throws BencodingException
	 *             if the bencoded object is incorrectly encoded.
	 */
	@SuppressWarnings("unchecked")
	public static final Object[] decodeDictionary(byte[] bencoded_bytes,
			int offset) throws BencodingException {
		...
		while (bencoded_bytes[offset] != (byte) 'e') {
			...
			offset = ((Integer) vals[0]).intValue();
			if (match) {
				info_hash_bytes = ByteBuffer
						.wrap(new byte[offset - info_offset]);
	 ...
	 *            the bencoded data.
	 * @param offset
	 *            the offset into {@code bencoded_bytes} that contains a
	 *            bencoded object.
	 ...
	 * @throws BencodingException
	 *             if {@code o} is not of type {@code HashMap},
	 *             {@code ArrayList}, {@code Integer}, or {@code ByteBuffer}.
	 */
	@SuppressWarnings("unchecked")
	public static final byte[] encode(Object o) throws BencodingException {
		if (o instanceof HashMap)
			...
			throw new BencodingException(
					"Error: Object not of valid type for Bencoding.");
	}
	 ...
	 */
	private static final byte[] encodeDictionary(
			HashMap<ByteBuffer, Object> dictionary) throws BencodingException {
		...
		int k = 0;
		for (Iterator<ByteBuffer> i = sorted_dictionary.keySet().iterator(); i
				.hasNext();) {
		...
		int offset = 1;
		for (int i = 0; i < dictionary_parts.length; i++) {
			System.arraycopy(dictionary_parts[i], 0, bencoded_dictionary,
